...
class CHACHA_GUTENBERG_ROUTE
{
    ...
    public function get_option_fields($request)
    {
        ...
        if (array_key_exists('subfield', $params) &&
            ...
            $return['value'] = get_field($params['field'], 'option');
        }
    ...
    public function get_all_options_fields($request)
    {
        ...
        foreach ($fields as $fieldname => $fieldvalue) {
            if ($fieldname == 'copyright') {
                $fields[$fieldname] = apply_filters('the_content', $fieldvalue);
            }
    ...
    public function get_author_from_postid($request)
    {
        ...
        if (count($fields['moe_user_avatar'])) {
            $fields['avatar_image'] = get_post($fields['moe_user_avatar'][0]);
        }
    ...
    public function get_breadcrumb($request)
    {
        ...
        if (array_key_exists('title', $params)) {
            ...
            $return[] = ['title' => $postObj->post_title];
        }
    ...
    public function get_form_by_id($request)
    {
        ...
        while (have_rows('customposttype_form_link', 'option')): the_row();
            if (get_sub_field('customposttype') == $val) {
                ...
                $parameters = '';
                if (get_class($post) == 'WP_Post' && $val == 'expert') {
                    $parameters = "expert_email=" . get_field('courriel_expert', $post);
                } elseif ($val == 'expert') {
                $parameters = 'expert_email=web@cha-cha.ca';
            }
